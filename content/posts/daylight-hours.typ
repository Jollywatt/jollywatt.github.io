#metadata((
  ...
  slug: "daylight-hours-a-spherical-geometry-problem",
...
#html.script(async: true, src: "https://www.desmos.com/api/v1.7/calculator.js?apiKey=dcb31709b452b1cf9dc26972add0fda6")
...
$
which, using $cos^2 λ = 1/2 (1 + cos 2λ)$ and $sin^2 λ = 1/2 (1 - cos 2λ)$, is the same as
$
...
$
  cos 2λ = (2cos^2φ - sin^2 ε - 1)/(sin^2 ε - 1)
  = 2 ( (sin^2 φ)/(cos^2 ε) ) - 1
$
where we use $cos^2 θ + sin^2 θ = 1$ twice to simplify.
Instead of solving directly for $λ$ here, note that the right-hand side matches the identity $cos 2λ = 2cos^2 λ - 1$, so we must have
$
  cos λ = (sin φ)/(cos ε)
  .
$
...
$
  λ = ±arccos((sin φ)/(cos ε)) + 2n pi
$
for some $n ∈ ℤ$. If we take just the positive value for $n = 0$ then we have $λ ∈ [0, pi]$, since this is the conventional range of $arccos$.
...
$
  θ = arctan(y/x) = arctan((sin λ)/(sin ε cos λ)) = arctan(1/(sin ε) tan λ)
  .
$
The arc length is thus twice this angle, $2θ$, while the total circumference is $2pi$. Therefore, written in full, the fraction of the day in daylight is
$
  L = 1/pi arctan(1/(sin ε) tan [arccos((sin φ)/(cos ))])
  .
$
...
To find $ε$ (the angle of the arc $overline(P N)$) we may use the spherical law of sines:
$
  (sin a)/(sin A) = (sin b)/(sin B) = (sin c)/(sin C)
$
Specifically, reading off the figure above, this gives us
$
  (sin overline(P N))/(sin(pi/2 - ω)) = (sin overline(O N))/(sin pi/2)
$
which reduces to
$
  (sin ε)/(cos ω) = sin ε_0
...
#html.script(
  ...
  function Figure4(id) {
  	...
  	let exprs = [
  		...
  		{
  			id: "north pole orbit",
  			latex: "\\left(-\\varepsilon_scos t,R\\left(o,-\\varepsilon_s\sin t,\cos\\varepsilon\\right)\\right)",
  			color: "#c74440",
  			lineWidth: "1",
  			domain: {
  				min: "0",
